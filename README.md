...
1. [正式設計・導入・運用ガイド Ver.0.0.3](docs/00-design-guide.md)
...
7. [Investigation Reports](docs/investigations/README.md)
...
Ver.0.0.3は設計・導入・運用文書の改訂番号であり、稼働ServerのRelease番号ではありません。稼働Serverの最初の正式Version番号は未決定で、現段階ではGitHub ReleaseおよびGit Tagを使用しません。
...
- 通常の既成Plugin検証はProject WayfarerへのIntegration Smoke Testを中心とする。
...
- Shared network currency: **Waymark (WM)**. Shopと報酬設計は後続タスクとする。
