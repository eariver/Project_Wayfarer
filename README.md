...
- EvenMoreFish 2.4.3: Main限定のCustom Fishing。`main`／`resource`だけで有効、専用MariaDBへJournal／統計を保存
...
- EvenMoreFishはMainだけに配置し、`main`／`resource`だけでCustom Fishを有効化する。Economy、Competition、Hunt、Lava／Void Fishingは無効とし、Credentialを含むRuntime ConfigはTemplateからRenderする。
...
- V0.2.x以降の独自Plugin構想は[Codex Task Instruction Archive](codex/README.md)内の参考草案として保存する。V0.1.0の実装対象・Release Blocker・開発開始承認ではない。
