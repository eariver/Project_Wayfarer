...
$portToken = '__WAYFARER_' + 'MARIADB_PORT__'
$targets = @(
    @{
        Name = 'Main'
        Template = 'servers/main/plugins/mcMMO/config.yml.template'
        Output = 'servers/main/plugins/mcMMO/config.yml'
    }
    @{
        Name = 'Frontier'
        Template = 'servers/frontier/plugins/mcMMO/config.yml.template'
        Output = 'servers/frontier/plugins/mcMMO/config.yml'
    }
)
...
foreach ($target in $targets) {
    ...
    if (-not (Test-Path -LiteralPath $templatePath -PathType Leaf)) {
        throw "Missing $($target.Name) mcMMO config template."
    }
    ...
    foreach ($token in @($userToken, $passwordToken, $portToken)) {
        ...
        if ($tokenCount -ne 1) {
            throw "Expected exactly one $($target.Name) mcMMO token of each type; found $tokenCount."
        }
        $rendered = $rendered.Replace($token, $replacementValues[$token])
    }

    if ($rendered.Contains($userToken) -or $rendered.Contains($passwordToken) -or $rendered.Contains($portToken)) {
        throw "An mcMMO token remained in the $($target.Name) output."
    }
    if ($rendered -notmatch '(?ms)^MySQL:\r?\n    Enabled: true\r?\n.*?^        Name: wayfarer_mcmmo\r?$') {
        throw "$($target.Name) mcMMO database selection validation failed."
    }
    if ($rendered -notmatch '(?m)^        TablePrefix: mcmmo_\r?$') {
        throw "$($target.Name) mcMMO table prefix validation failed."
    }
    if ($rendered -notmatch '(?ms)^    Server:\r?\n        SSL: true\r?\n        Port: [0-9]+\r?\n        Address: 127\.0\.0\.2\r?$') {
        throw "$($target.Name) mcMMO server endpoint validation failed."
    }
    if ($rendered -notmatch '(?ms)^Scoreboard:\r?\n.*?^    UseScoreboards: false\r?$') {
        throw "$($target.Name) mcMMO scoreboard policy validation failed."
    }
...
foreach ($plan in $renderPlans) {
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Rendered $($plan.Name) mcMMO config is not ignored by Git."
        }

        $completed.Add($plan.Name)
        Write-Host "$($plan.Name) mcMMO configuration rendered successfully."
    }
    catch {
        ...
        throw "$($plan.Name) mcMMO configuration rendering failed. $completedSummary The failing component may have an incomplete output. $($_.Exception.Message)"
    }
}
