...
外部拡張Pluginが未導入または互換性未確認の場合、対象機能は無効として扱います。Main、Frontier、Lobbyの基本的な起動、接続、通常インベントリの分離、WaymarkおよびmcMMOの既存設計を、未導入の外部拡張へ依存させません。GUI、Command、Portalまたは権限から未実装機能へ到達できない構成にします。
