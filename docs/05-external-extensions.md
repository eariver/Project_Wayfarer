...
外部拡張Pluginが未導入または互換性未確認の場合、対象機能は無効として扱います。Main、Frontier、Lobbyの基本的な起動、接続および通常Inventoryの分離を未導入の外部拡張へ依存させません。WaymarkとmcMMO共有は設計済みの将来計画であり、現時点では未導入です。GUI、Command、Portalまたは権限から未実装機能へ到達できない構成にします。
