...
- BetterStructures 2.6.3 is currently installed with the vanilla-like `103 Default Structures` pack; structures appear only when Persistent Main generates new chunks. Ver.0.0.6 plans a five-Pack replacement baseline after separate Content and Resource Pack preflight, but no World is regenerated by this guide.
