...
This guide separates the current playable state from the future V0.1.0 Alpha target. Permission Phase 1A and the current Main terrain baseline are available, but Ver.0.0.6 does not mean that the replacement Main generation, Hubs, Gates, Main Spawn protection, Builder allowlist, CoreProtect, MVI, Resource Packs, custom Plugins, or Frontier Themes are already available.
...
- BetterStructures 2.6.3 now loads the verified five-Pack 278-Structure working set with the BetterStructures Prop Pack and Main Resource Pack. These structures appear only in newly generated Persistent Main chunks; the existing terrain still reflects the former `103 Default Structures` baseline until the separately approved replacement generation. No World is regenerated by this guide.
...
- The current natural Main spawn and bedless respawn baseline is the approved Savanna location around `(320, 70, 128)`. It remains authoritative until the replacement baseline passes. The permanent spawn Hub, Lobby/Frontier Return Gates, and three Resource Gates are V0.1.0 targets and are not built yet.
