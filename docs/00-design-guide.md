# Project Wayfarer 設計・導入・運用ガイド Ver.0.0.6
...
|区分|値|
|---|---|
|Document revision|Ver.0.0.6|
|更新日|2026-07-25|
|Runtime software versions|`versions.yml`と`plugin-manifest.yml`を正本とする|
|Target server release|V0.1.0 Alpha（未達）|

Ver.0.0.6は、現在の実装済みBaselineを維持しながら、Main Content拡張と再生成、Ruined Frontier／Worlds Beyondの両Theme、MVI、外部Custom PluginおよびResource Pack配信をV0.1.0 Alphaの正式Scopeへ反映する文書Revisionです。稼働ServerのRelease番号ではなく、`V0.1.0 Alpha`は未達です。Baseline Backupと隔離Restoreを含むBlocker完了時にGit Tag／GitHub Releaseの採用を最終決定します。
...
- Mainは恒久生活、建築、保管およびバニラに近い通常Survivalの拠点とする。
...
|---|---|---|---|
|通常Inventory|Local|Main Local|Frontier内はMVI Group別。Mainとは完全分離|
|Ender Chest／XP／Health／Food|Local|Main Local|Frontier内はMVI Group別。Mainとは完全分離|
...
Builder所属は2時間、Admin所属は30分を標準運用値としますが、Ver.0.0.6でも技術的に固定しません。実行時に期限を指定し、作業終了時は期限前でも自己降格します。EligibilityはVelocity上の`/lpv`だけへ限定され、永続Parent追加、他人へのRole操作、誤ったRoleおよび任意Group操作を拒否します。複数RoleへのTemporary Parent所属は同時に保持しない運用とします。
...
Phase 1Bの`wayfarer_builder`は明示Allowlist方式です。候補となるWorldEdit、Creative／Survival／Spectator切替、Teleport、MultiverseおよびGate操作は、Ruined Frontier／Worlds Beyond、MVI、ResourcePackManager、EliteMobs、Wayfarer custom Plugin、採用Gate方式およびBuilder担当作業の確定後、採用Versionの実Commandから必要な操作だけを列挙します。
...
- Frontier Gate Hub: Lobby Return、Main Hub、Beyond Gate、Guild Gate、両Themeからの安全な帰還地点、既存保護、最低限の案内。
