...
Lobbyと`frontier_gate`は`__global__` Regionで`passthrough=deny`とし、`build` Flagは使用しません。現行Runtimeでは`wayfarer_builder`だけが建築でき、WorldEdit／WorldGuard権限は`server=lobby`および`server=frontier` Contextに限定します。MainにはPluginを配置していますが、Project固有Region、Global Flag、MemberおよびOwnerはまだ設定していません。Ver.0.0.4の最終Builder Allowlistは後続の権限実装タスクで置き換え・拡張し、現行状態を実装済みと混同しません。
