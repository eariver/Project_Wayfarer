# Project Wayfarer — Ruined Frontier仕様書 V0.0.5
...
| Primis／Instance | EliteMobs Content PackageおよびMVI対応方式で管理 |

World IDはFrontier共通仕様と導入Configで固定し、本書のIDを初期Baselineとして参照する。Inventory Group割当のRuntime正本はMultiverse-Inventoriesとする。

要件：

- Frontier LobbyにはAdventurer's Guildへ接続する物理Guild Gateを配置する。V0.0.5ではTheme選択GUIを使用しない。
...
`Multiverse-NetherPortals`をFrontier Backendへ導入し、Ruined Frontier三次元を明示的なWorld Familyとして**双方向Link**する。命名規則による自動Linkだけに依存しない。
...
Guild系Worldは、Multiverse-Inventoriesの同一`guild` Groupへ登録する。
...
通常Player Stateの保存・復元はMVIへ委譲し、`Wayfarer_Frontier`はGuild Inventoryを独自DBへ保存しない。

EliteMobs InstanceのInventory共有方式は、次の優先順で決定する。
...
MVI／導入Config上でRuined Frontier三次元に属さない全World
未知の新規World
```
...
```text
frontier_gate
MVI／導入Config上でGuild Groupに属さない全World
```

EliteMobs Instanceの生成、有効化および削除はEliteMobsのContent Package Lifecycleを正とする。Inventory共有は、固定WorldのMVI静的登録、承認済みBlueprint限定Regex、必要時のみEliteMobs–MVI Adapterの順で決定する。Adapter採用時もMVI Groupへの追加・解除だけを行い、EliteMobsの内部Configを直接変更しない。Worlds Beyondでは、EliteMobsのWorld設定とWorldGuardの両方でElite Spawn／Eventを防止する。
...
- ResurrectionChest等はV0.0.5必須としない。
...
Ruined Frontier alphaでは、`Wayfarer_Frontier`を必須のInventory管理Pluginとして扱わない。
...
1. 導入するEliteMobs VersionとContent Packageを固定する。
2. 固定Worldか動的Instance Worldか確認する。
3. 固定WorldならMVI Guild Groupへ静的登録する。
4. 動的Worldなら承認済みBlueprint名＋連番に限定した厳密Regexを試験する。
...
- Primis／Dungeon入口はGuildのNPC、Portalまたは承認済み導線を基本とする。
- `/em quit`、Package固有退出、死亡Respawnおよび強制TeleportでMVIのGroup切替が正常に働くことを試験する。
- `/mv tp`その他の管理Teleport権限を一般Playerへ付与しない。
- Admin TeleportでもMVIが正しいGroup Profileを適用することを試験する。
- 通常運用でOPを使用しない。

LuckPerms設定例は仕様上の意図を示す参考とし、正式CommandとNode構成は導入Versionで再確認する。
...
- Frontier LobbyのTheme選択GUIまたは物理Guild Gateの移動処理
- 通常Inventory、Ender Chest、XP、Health、Foodの保存・復元
- 通常のTeleport、Portal、RespawnおよびWorld変更の包括監視
- EliteMobs Instance Worldの生成
- EliteMobs InstanceのDungeon Gameplay／Package Lifecycle／Player退出／World削除
- EliteMobs内部Config／Database／Content Package Fileの変更
- EliteMobs Boss AI
- EliteMobs Loot生成
...
- [ ] Multiverse-InventoriesのGuild Groupへ固定Worldが登録される。
...
- [ ] Gate、Portal、Respawn、Admin Teleportおよび再接続でMVI Profileが正しく切り替わる。
...
- [ ] Ruined Frontier Overworld／NetherのLinkが双方向である。
- [ ] Ruined Frontier Overworld／EndのLinkが双方向である。
- [ ] Stronghold End PortalはRuined Frontier Endへ接続する。
- [ ] End Exit PortalはRuined Frontier Overworldへ帰還する。
...
- [ ] Ruined FrontierのPortalが`frontier_iris`へ接続されない。
...
- [ ] Instanceの生成、有効化、Gameplay、Player退出、終了およびWorld削除はEliteMobsのContent Package Lifecycleが管理する。
- [ ] 対象Contentが固定World、複製World、一時Worldのどれか判定できる。
- [ ] Blueprint名とInstance World命名規則を記録する。
- [ ] 固定WorldはMVI Guild Groupへの静的登録でInventoryを共有できる。
- [ ] 動的Worldは承認済みBlueprint名に限定した厳密Regexを試験する。
- [ ] 同時Instance、終了、Restart、再接続および削除後もInventoryが失われない。
- [ ] Regexで安全に成立する場合はAdapterを実装しない。
- [ ] Regexで不足する場合だけ生成／削除EventとMVI APIを用いるAdapter要件を確定する。
...
19. Guild Gateの実装方式と到着地点
20. Multiverse-NetherPortals正式Version
21. Overworld／NetherおよびOverworld／Endの双方向Link設定方法
22. `handle-end-exit-respawn: true`
23. End Exit時のBed／Respawn Anchor処理
24. BetterStructures内End Portalの接続・帰還先
25. EliteMobs Content Package LifecycleをInstance Lifecycleの正本とすること
26. 対象Dungeonが固定World／複製World／一時Worldのどれか
27. Blueprint World名とInstance World命名規則
28. MVI静的登録の可否
29. 承認済みBlueprint名限定Regexの書式と適用Timing
30. 同時Instance／終了／Restart／再接続／削除のSmoke Test
31. **EMアダプタの必要性判断**
32. Adapter採用時のInstance生成Event
...
40. `/em quit`およびPackage固有退出CommandでのMVI挙動
41. Reset／Season方針

これらはV0.0.5仕様を破棄する未決事項ではなく、ArtifactおよびRuntime依存の実装Lock項目である。
...
| Instance／Adapter | 静的MVI登録または厳密Regexを優先 | Instance方式、同時生成、退出、Restart、削除を試験し、Adapterの必要性を判断 |
...
9. 通常Inventoryの保存・復元はMultiverse-Inventoriesへ委譲する。
10. EMアダプタは静的登録または厳密Regexで不足すると確認された場合だけ実装する。
11. EliteMobs／BetterStructuresのDBや内部FileをWayfarer Pluginから直接変更しない。
12. World削除、Trim、Resetは独立した破壊的タスクとして扱う。
...
- Frontier共通仕様の参照先をV0.0.5へ更新した。
- Ruined FrontierのOverworld／Nether／End構成、Multiverse-NetherPortals LinkおよびGameplay Scopeは変更していない。
- Portal Acceptanceを、Overworld単一WorldとなったWorlds Beyondの`frontier_iris`へ誤接続しない表現へ更新した。
