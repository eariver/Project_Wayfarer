# Project Wayfarer — Frontierサーバ仕様書 V0.0.5
...
>
> - [Worlds Beyond仕様書 V0.0.6](./Worlds_Beyond_Specification_V0.0.6.md)
> - [Ruined Frontier仕様書 V0.0.5](./Ruined_Frontier_Specification_V0.0.5.md)
>
...
| Worlds Beyond | `frontier_iris`（Overworld単一World） |
...
- V0.0.5ではTheme選択GUIを実装しない。GUI化は将来の利便性機能としてDeferredへ置く。
...
- Worlds BeyondはOverworld単一Worldとし、Nether／End Portalを移動導線として使用しない。
- Worlds BeyondのPortal操作をDefault Nether／End、Ruined Frontier、Mainまたは未知WorldへのFallback経路にしない。
...
- World IDはMultiverse-Core、MVI、BetterStructures、EliteMobs、IrisおよびWorldGuardの各Configで一致させる。Multiverse-NetherPortalsとの一致確認はRuined Frontier Familyだけを対象とする。
...
`Wayfarer_Frontier`はこれらのEventを包括的に横取りしてInventoryを独自切替しない。各経路でMVIが正しく機能することを受入試験し、問題がある経路はPermission、移動導線または外部Plugin Configで閉じる。Worlds BeyondではNether／End Portal自体を移動経路として許可しない。
...
- `Multiverse-NetherPortals`はRuined Frontier FamilyについてOverworld／NetherおよびOverworld／Endを双方向で登録する。
- Ruined Frontierでは`handle-end-exit-respawn: true`をBaselineとする。
- Ruined Frontier End Exit Portalの帰還先はRuined Frontier Overworldに限定する。
- 別Themeに設定されたBed／Respawn AnchorをCross-theme帰還先として使用させない。
- Ruined Frontierの実Bukkit World名をMultiverse-Core、MVI、Multiverse-NetherPortals、BetterStructuresおよびEliteMobs Configで一致させる。
- Worlds Beyond向けMultiverse-NetherPortals Linkを作成しない。
- `frontier_iris`ではNether／End Portalの点火または通過を拒否し、Default World、Ruined Frontier、Mainまたは未知WorldへFallbackさせない。
- Ruined FrontierのPortalを`frontier_iris`へ接続しない。
...
- V0.0.5のWorlds Beyondは専用Modelを必須としない。
...
- Worlds BeyondのWaystoneは通常破壊から保護する。Launchpadは環境要因から保護し、仕様に沿ったPlayer手動Breakだけを許可してItemをDropしない。
...
ResurrectionChest等の死亡補助は後続Phaseとし、V0.0.5では前提にしない。
...
- Ruined Frontier向けMultiverse-NetherPortals World Family Link
- Worlds Beyond Nether／End Portal禁止
- WM Adapter
- Permission境界
...
- [ ] Worlds Beyond Groupに`frontier_iris`だけが登録され、Nether／End／未知Worldが含まれない。
...
- [ ] Multiverse-NetherPortalsの正式VersionをRuined Frontier向けに固定できる。
- [ ] Ruined FrontierのOverworld／Nether Linkが双方向である。
- [ ] Ruined FrontierのOverworld／End Linkが双方向である。
...
- [ ] `frontier_iris`でNether Portalを点火または通過できない。
- [ ] `frontier_iris`のEnd PortalからEndへ通過できない。
- [ ] Worlds BeyondのPortal操作がDefault Nether／End、Main、Ruined Frontierまたは未知WorldへFallbackしない。
- [ ] Portal経由でMVIの不要なProfile変更が発生しない。
- [ ] Ruined FrontierにおけるMob、Item、VehicleのPortal通過挙動が許容範囲である。
...
- Gate到着地点および帰還導線
- Theme選択GUIをV0.0.5で実装しないこと

Portal：

- Multiverse-NetherPortalsの正式Version（Ruined Frontierのみ）
- Ruined Frontier Overworld／NetherおよびOverworld／Endの双方向Link
- Ruined Frontierの`handle-end-exit-respawn: true`
- Ruined Frontier End Exit時のBed／Respawn Anchor処理
- BetterStructures内End Portalの接続・帰還先
- Worlds Beyond Allowlistを`frontier_iris`だけに固定する方法
- `frontier_iris`でNether Portalの点火／通過を拒否する方法
- `frontier_iris`でEnd Portalの通過を拒否する方法
- Worlds Beyond Portal操作のDefault／Cross-theme Fallback禁止
...
| Portal | Ruined Frontier Familyの往復先、End Exit、Vehicle／Mob挙動、Worlds BeyondのPortal禁止とFallback防止 | Multiverse-NetherPortals Config／Portal禁止Config |
...
12. V0.0.5の暫定値はConfigから変更可能にする。
13. 実運用Dataを得た後に価格、期間、生成密度および難易度を調整する。
14. Worlds BeyondのWorld Allowlistは`frontier_iris`だけに固定し、Nether／End／未知Worldを自動採用しない。
...
- Worlds Beyondを`frontier_iris`単一Worldへ変更した。
- Worlds Beyond Nether／Endを論理導線、MVI Group、Portal LinkおよびRoadmapから削除した。
- Iris Netherの公式未完成警告とProject OwnerによるEnd非採用判断をTheme仕様へ委譲した。
- Multiverse-NetherPortalsの責務をRuined Frontier Familyへ限定した。
- Worlds BeyondのNether／End Portalを禁止し、Default Worldまたは他ThemeへのFallbackを禁止した。
- Portal／MVI／Gate／Roadmap／Acceptance／実装前Lock項目を単一World構成へ同期した。
- LaunchpadのPlayer手動BreakをFrontier共通保護方針へ同期した。
