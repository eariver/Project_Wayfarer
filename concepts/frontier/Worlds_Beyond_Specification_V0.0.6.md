# Project Wayfarer — Worlds Beyond仕様書 V0.0.6

> **状態：設計仕様／実装前**
>
> 本文書は、Frontier Backend上のIris Theme **Worlds Beyond**のWorld構成、Gameplay、独自Plugin機能、Database、Economyおよび受入試験を定義する。
>
> Frontier共通仕様は、[Frontierサーバ仕様書 V0.0.5](./Frontier_Server_Specification_V0.0.5.md)を参照する。
>
> 独自Plugin責務と実装境界は、[`Project_Wayfarer_Worlds_Beyond_Plugin_Concept_v0.0.4.md`](../plugins/frontier/Project_Wayfarer_Worlds_Beyond_Plugin_Concept_v0.0.4.md)をDesign Inputとして同期する。
>
> 数値はV0.0.6の初期Balance Baselineであり、すべてConfigから変更可能とする。
...
Worlds Beyondは、Irisによる**Overworld単一World**として構成する。
...
- Iris公式Organization `IrisDimensions`の`nether` Repositoryは、READMEでNether Dimensionが未完成で使用不能であると明記しているため採用しない。
- Iris EndもProject Owner判断により採用しない。
- `frontier_iris_nether`および`frontier_iris_the_end`は作成、登録、LinkまたはWorlds Beyond Allowlistへの追加を行わない。
- Worlds Beyond機能は`frontier_iris`だけで有効化し、Prefix、部分一致または広いWildcardで未知Worldを自動採用しない。

World IDはProjectの導入Configで固定し、Multiverse-Core、Multiverse-Inventories、IrisおよびWorlds Beyond機能Allowlistで一致させる。Inventory Group割当のRuntime正本はMultiverse-Inventoriesとする。
...
V0.0.6ではTheme選択GUIを使用せず、Frontier Lobbyに設置するBeyond Gateを正規入口とする。Gateの実装方式はFrontier共通仕様でLockする。

### 2.1 Nether／End非採用方針

Worlds BeyondにはNetherおよびEndの移動先を設けない。

- Worlds Beyond向け`Multiverse-NetherPortals` Linkを作成しない。
- `frontier_iris`内のNether Portalは、点火または通過をRuntime Config、Region保護または採用Portal機構で拒否する。
- Stronghold等にEnd Portal Frameが存在する場合も、Endへの通過を許可しない。
- Portal操作をDefault Nether／Default End、Ruined Frontier、Mainまたは未知WorldへのFallback経路にしない。
- 生成済みのPortal状構造が存在しても、Worlds Beyondでは装飾または不活性構造として扱う。
- 禁止方法の具体的なPlugin／Configは実装タスクでLockし、`Wayfarer_Frontier`へ通常Portal処理を不用意に再実装しない。

Ruined FrontierのOverworld／Nether／End構成とPortal Linkは、Ruined Frontier仕様およびFrontier共通仕様を正とし、本方針の影響を受けない。
...
- `frontier_iris`だけをIris Overworldとして生成する。
- Iris NetherおよびIris Endは生成・登録しない。
- 管理者が手作業でLandmarkを配置しない。
- Iris以外のStructure GeneratorをV0.0.6では追加しない。
...
`frontier_iris`だけをMultiverse-Inventoriesの`worlds_beyond` Groupへ登録する。
...
`frontier_iris`への初回入場時に、`Wayfarer_Frontier`が次を支給する。ここでいうTraversal Loadoutは`Wayfarer_Core`とは無関係な、Worlds Beyond用の基本移動装備を意味する。恒久支給品と消耗品を区別し、無料再支給による複製を防止する。
...
**LeafGrapple 1.0.2**をV0.0.6の第一候補とする。
...
| Teleport | Playerが`frontier_iris`内、Destinationが発見済みかつPROTECTEDまたはCONTESTABLE | 0 WM | Destinationへ移動 |
...
- Interaction Block
- 状態表示用Light／Particle
- Worlds Beyond Overworld用Block Palette
...
- Playerの現在WorldとDestinationがともに`frontier_iris`であり、現在Teleport可能である。
...
- Playerの現在WorldとDestinationがともに`frontier_iris`である。
...
V0.0.6では集計UIを実装しないが、Dataを失わないSchemaとする。
...
- Mainと競合する恒久生活拠点を禁止するHard RuleはV0.0.6で設けない。
...
- [ ] `frontier_iris`が選定したIris Overworld Engine／Pack／Seedで生成される。
- [ ] Worlds Beyond対象Worldが`frontier_iris`だけである。
- [ ] `frontier_iris_nether`および`frontier_iris_the_end`が作成・登録・Allowlist追加されない。
- [ ] 任意のNether／The End／未承認WorldでWorlds Beyond固有機能が有効化されない。
- [ ] Prefix、部分一致または広いWildcardで未知Worldを自動採用しない。
- [ ] Worlds Beyond向けMultiverse-NetherPortals Linkを要求しない。
- [ ] `frontier_iris`内でNether Portalを点火または通過できない。
- [ ] Stronghold等のEnd PortalからEndへ通過できない。
- [ ] Portal操作がDefault Nether／End、Ruined Frontier、Mainまたは未知WorldへFallbackしない。
...
- [ ] Multiverse-InventoriesのWorlds Beyond Groupへ`frontier_iris`だけが登録される。
- [ ] `frontier_iris`でWorlds Beyond GroupのPlayer Stateが使用される。
- [ ] Frontier Lobby、Main、Ruined FrontierとInventoryが分離される。
- [ ] Beyond Gate、Return Gate、Respawn、Admin Teleportおよび再接続でMVI Profileが正しく切り替わる。
...
- [ ] `frontier_iris`以外のWorldへTeleportできない。
...
次の値はV0.0.6の初期BaselineとしてConfig化し、本流のProject構築チャットで実プレイ結果を基にLockする。評価は`frontier_iris`内の平地、山岳、渓谷、洞窟、浮遊地形および海洋等で行い、移動時間、失敗率、WM収支、設置物密度およびServer負荷を記録する。
...
| Performance | Persistent単一World | Iris生成、Waystone検索、構造物保護、Launchpad累積の負荷 |
...
Portal／World境界実装Lock：

- Worlds Beyond Allowlistを`frontier_iris`だけに固定する方法
- Nether Portalの点火／通過を拒否する実装方式
- End Portalの通過を拒否する実装方式
- Default Nether／End、Ruined Frontier、Mainまたは未知WorldへのFallbackがないこと
- Multiverse-Core、MVI、Irisおよび機能別AllowlistのWorld ID一致
...
- Worlds BeyondをIris Overworld単一Worldへ変更し、対象Runtime IDを`frontier_iris`だけとした。
- Iris公式`IrisDimensions/nether` READMEの未完成・使用不能警告を根拠にIris Netherを非採用とした。
- Project Owner判断によりIris Endも非採用とした。
- `frontier_iris_nether`および`frontier_iris_the_end`の生成、登録、AllowlistおよびPortal Linkを削除した。
- Worlds Beyond向けMultiverse-NetherPortals連携を削除し、Nether／End Portalを不活性化する方針を追加した。
- MVI Group、Traversal Loadout発行、Waystone、Teleport、Launchpad、ShopおよびNavigation機能の対象を`frontier_iris`へ限定した。
- Portal操作がDefault World、Ruined Frontier、Mainまたは未知WorldへFallbackしない要件を追加した。
- Config、Future Phase、Acceptance、PlaytestおよびDeferred項目を単一World構成へ同期した。
- V0.0.5で確定したLaunchpadおよびNavigation GUIの中核仕様は変更していない。
- Navigation ItemからのTeleportはOrigin Waystoneを要求せず、Playerの現在WorldとDestinationを`frontier_iris`へ限定する。
