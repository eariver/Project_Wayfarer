# Project Wayfarer — Frontierサーバ仕様書 V0.0.2
...
>
> - [Worlds Beyond仕様書 V0.0.2](./Worlds_Beyond_Specification_V0.0.2.md)
> - [Ruined Frontier仕様書 V0.0.2](./Ruined_Frontier_Specification_V0.0.2.md)
>
...
11. 動的Instanceは許可済みContent Package IDと生成Eventまたは確実な検出手段に基づき一時登録し、World名Prefixだけでは承認しない。
12. Instance終了時はPlayer退避、Inventory保存、登録解除、World削除の順序を保証する。

MariaDBを正本とし、必要に応じてRedisをLock／Cacheに使用する。RedisだけをInventoryの永続正本にしない。

権限境界：

- EliteMobsのDungeon直接Teleport、Spawn Teleport等のCross-group移動権限を一般Playerへ付与しない。
...
| Multiverse-NetherPortals | 各Theme World FamilyのOverworld／Nether／End接続管理 |
...
- V0.0.2のWorlds Beyondは専用Modelを必須としない。
...
ResurrectionChest等の死亡補助は後続Phaseとし、V0.0.2では前提にしない。
...
`Wayfarer_Frontier`は、許可済みContent Package ID、生成Eventまたは確実な検出手段を用いるEliteMobs Instance Registryを持つ。未知WorldはPrefix一致だけでGuild Groupへ登録せず、Neutral扱いまたは入場拒否としてFail-closedする。Instance削除前にPlayerを安全地点へ退避させ、Inventory保存完了と登録解除を確認する。
...
- [ ] Worlds BeyondおよびRuined FrontierのNether Portal往復先が、それぞれ同一ThemeのWorld Familyである。
- [ ] Stronghold End Portalが同一ThemeのEndへ接続する。
- [ ] End Exit Portalが同一ThemeのOverworldへ帰還する。
- [ ] BetterStructures内に配置されたEnd Portal BlockがRuined Frontier Familyへ正しく接続する。
- [ ] Worlds BeyondとRuined FrontierのPortalが相互接続されない。
- [ ] Mob、Item、VehicleのPortal通過挙動が許容範囲である。
- [ ] 同一Inventory Group内のPortal移動で不要な保存／復元が発生しない。
