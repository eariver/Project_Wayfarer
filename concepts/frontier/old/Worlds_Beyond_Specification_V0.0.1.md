# Project Wayfarer — Worlds Beyond仕様書 V0.0.1
...
>
> 数値はV0.0.1の初期Balance Baselineであり、すべてConfigから変更可能とする。
...
Runtime IDは実装前にBukkit上の実World名と一致させる。EndのAliasと実Physical Nameが異なる場合、実Bukkit名をAllowlistの正本とする。
...
- Iris以外のStructure GeneratorをV0.0.1では追加しない。
...
**LeafGrapple 1.0.2**をV0.0.1の第一候補とする。
...
V0.0.1では集計UIを実装しないが、Dataを失わないSchemaとする。
...
- Mainと競合する恒久生活拠点を禁止するHard RuleはV0.0.1で設けない。
...
- [ ] Frontier Gateへの安全な帰還が可能。

### Inventory

- [ ] Worlds Beyond三次元でInventoryが共有される。
- [ ] Frontier Gate、Main、Ruined FrontierとInventoryが分離される。
