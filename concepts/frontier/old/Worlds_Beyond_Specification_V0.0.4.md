# Project Wayfarer — Worlds Beyond仕様書 V0.0.4
...
>
> 数値はV0.0.4の初期Balance Baselineであり、すべてConfigから変更可能とする。
...
V0.0.4ではTheme選択GUIを使用せず、Frontier Lobbyに設置するBeyond Gateを正規入口とする。Gateの実装方式はFrontier共通仕様でLockする。
...
- Iris以外のStructure GeneratorをV0.0.4では追加しない。
...
**LeafGrapple 1.0.2**をV0.0.4の第一候補とする。
...
V0.0.4では集計UIを実装しないが、Dataを失わないSchemaとする。
...
- Mainと競合する恒久生活拠点を禁止するHard RuleはV0.0.4で設けない。
...
次の値はV0.0.4の初期BaselineとしてConfig化し、本流のProject構築チャットで実プレイ結果を基にLockする。評価は地上、山岳、渓谷、洞窟、Nether、Endなど複数地形で行い、移動時間、失敗率、WM収支、設置物密度およびServer負荷を記録する。
...
10. System Structureは物理破壊ではなくLifecycleで管理する。
