# Project Wayfarer — Worlds Beyond仕様書 V0.0.2
...
>
> 数値はV0.0.2の初期Balance Baselineであり、すべてConfigから変更可能とする。
...
`Multiverse-NetherPortals`で三次元を明示的なWorld FamilyとしてLinkし、命名規則による自動Linkだけに依存しない。Stronghold End Portal、End Exit Portal、Mob／Item／VehicleのPortal通過を実機確認する。Ruined Frontier Familyとは相互接続しない。
...
- Iris以外のStructure GeneratorをV0.0.2では追加しない。
...
**LeafGrapple 1.0.2**をV0.0.2の第一候補とする。
...
V0.0.2では集計UIを実装しないが、Dataを失わないSchemaとする。
...
- Mainと競合する恒久生活拠点を禁止するHard RuleはV0.0.2で設けない。
...
- [ ] Overworld／Netherの往復先がWorlds Beyond Family内である。
...
次の値はV0.0.2の初期BaselineとしてConfig化し、本流のProject構築チャットで実プレイ結果を基にLockする。評価は地上、山岳、渓谷、洞窟、Nether、Endなど複数地形で行い、移動時間、失敗率、WM収支、設置物密度およびServer負荷を記録する。
