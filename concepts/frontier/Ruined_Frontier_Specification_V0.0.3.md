# Project Wayfarer — Ruined Frontier仕様書 V0.0.3
...
Primis付属DungeonおよびEliteMobs Instanceの生成、有効化、Gameplay、Package固有Respawn、終了およびWorld削除は、EliteMobsのContent Package Lifecycleを正とする。`Wayfarer_Frontier`のInstance Boundary Registryは、許可済みContent Packageから生成されたWorldであることを検証し、Guild Inventory GroupおよびTheme境界へ一時登録する。未知WorldをPrefix一致だけで自動承認せず、EliteMobsの内部Config、DatabaseまたはContent Package Fileを直接変更しない。
...
EliteMobs Instanceの生成、有効化および削除はEliteMobsのContent Package Lifecycleを正とする。`Wayfarer_Frontier`のInstance Boundary Registryは、許可済みPackageから生成されたWorldであることを検証し、Guild Inventory GroupおよびTheme境界へ一時登録する。EliteMobsの内部Configを直接変更しない。Worlds Beyondでは、EliteMobsのWorld設定とWorldGuardの両方でElite Spawn／Eventを防止する。
...
- ResurrectionChest等はV0.0.3必須としない。
...
- 許可済みPackageの生成元検証
- Instance WorldのGuild Group一時登録／解除
- Theme Item Boundary適用
- Instance退出時のPlayer退避とInventory保存順序保証
- Crash／Restart後の孤立Instance、孤立Lockおよび未完了Transition検出
- Theme固有Item漏出防止
- WM Adapter
- 初回案内
...
- Permission拒否だけをInventory境界の安全性根拠にしない。
- `/em quit`、Package固有退出、死亡Respawnおよび強制TeleportもEvent側でGroup境界を検査する。
...
- EliteMobs InstanceのDungeon Gameplay／Package Lifecycle／World削除
...
18. Overworld／NetherおよびOverworld／Endの双方向Link設定方法
19. `handle-end-exit-respawn: true`
20. End Exit時のBed／Respawn Anchor処理
21. BetterStructures内End Portalの接続・帰還先
22. EliteMobs Content Package LifecycleをInstance Lifecycleの正本とすること
23. Instance生成／終了を検出するEventまたは代替手段
24. Content Package IDの検証方法
25. Guild Inventory Groupへの一時登録／解除方法
26. Instance削除前のPlayer退避先
27. Crash／Restart後の孤立Instance検出方法
28. EliteMobs内部Config／Database／Content Packageを変更しないこと
29. InstanceのCoreProtect／Backup対象範囲
30. `elitemobs.user`の正式な子Permission構成
31. 明示拒否するTeleport Permission Node
32. Guild NPC／Portalによる正規移動導線
33. `/em quit`およびPackage固有退出Commandの境界検査方法
34. Reset／Season方針

これらはV0.0.3仕様を破棄する未決事項ではなく、ArtifactおよびRuntime依存の実装Lock項目である。
...
| Instance Lifecycle | EliteMobs Lifecycle＋Instance Boundary Registry | 生成・退出・再接続・Restart・削除時のInventory安全性と責務分界 |
