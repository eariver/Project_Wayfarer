# Project Wayfarer Worlds Beyond Plugin Concept v0.0.3

> **状態:** Under Review  
> **保存先:** Project Wayfarer Repository `concepts/plugins/frontier/`  
> **関連Concept:** `../Project_Wayfarer_Plugin_Concept_v0.0.3.md`  
> **関連Theme Concept:** `concepts/frontier/Worlds_Beyond_Specification_V0.0.5.md`（Frontier検討チャットで同期改訂）  
> **対象Runtime:** Frontier  
...
- Safe Arrival
- Player状態
- World／Theme Allowlist

Navigation Item自体が座標を保持して直接Teleportするのではなく、GUI入口とし、Teleport処理はWaystone Domainと安全判定を経由する。
...
- Teleport GUIが発見済みWaystoneと安全条件を検証する
...
Project Ownerの新判断として、次を`concepts/frontier/Worlds_Beyond_Specification_V0.0.5.md`へ同期する。
...
Theme Concept v0.0.4の回収仕様と「Navigation ItemはTeleportに使用しない」という記述は、新判断に合わせて置き換える。

---

## 26. v0.0.3変更概要

- Navigation Itemから統合Navigation GUIを開き、Teleport GUIを含める仕様へ変更した。
- Teleport対象を発見済みでTeleport可能なWaystoneとし、Waystone Domainの安全判定を必須とした。
- Launchpadを単一Blockの金の感圧板`LIGHT_WEIGHTED_PRESSURE_PLATE`とする初期Baselineを追加した。
- 金の感圧板へ乗ると起動し、Sneak中は起動しない仕様を追加した。
- Custom Modelを初期必須としない方針を追加した。
- Launchpad Itemが通常の死亡Drop対象であり、無料再支給対象外であることを明記した。
- `max-active-per-player: 0`を初期Balanceへ追加した。
- 使用感FeedbackによりMaterial、Trigger、速度等をv0.0.xで調整できることを明記した。
- Worlds Beyond Theme Concept v0.0.5へ同期すべきDeltaを明記した。
