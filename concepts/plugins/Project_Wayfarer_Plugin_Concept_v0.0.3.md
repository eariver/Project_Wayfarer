...
1. Project Ownerの明示判断をConcept改訂またはDecision Recordへ反映した最新記録
2. Project Wayfarer現行`docs/`
3. Project Wayfarer現行Server／Theme Concept
4. 本Plugin Concept群
5. 旧Conceptおよび引継ぎ資料

会話上の判断がRepository文書へ未反映の場合は、Concept内で「正本同期待ちのDelta」として明示し、実装Taskを開始する前にProject側文書へ反映する。
...
```text
concepts/plugins/
├─ Project_Wayfarer_Plugin_Concept_v0.0.3.md
├─ main/
...
└─ wayfarer-elitemobs-mvi-contract
   （Decision Resultが`ADAPTER_REQUIRED`の場合だけ作成）
...
- `main/Project_Wayfarer_Growth_Tool_Concept_v0.0.5.md`
...
Worlds Beyond Launchpadの初期Player体験は、単一の金の感圧板を踏んで起動し、Sneak中は起動しないBaselineとする。使用感はテストサーバでFeedbackを収集し、最終仕様をv0.0.xで調整する。
