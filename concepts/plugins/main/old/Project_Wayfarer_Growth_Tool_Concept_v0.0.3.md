# Project Wayfarer Growth Tool Concept v0.0.3
...
- Material変更時は、新Materialの最大耐久値へ同じ消費率を換算する。
...
- Upgrade履歴保持Toolが再びDiamond条件へ到達するとWM再請求なしでNetheriteへ復帰する
...
- Config再計算でMaterialを変換する際、ACTIVE Toolの換算後残存耐久値を最低`1`へClampすることを明文化した。
- Netherite化を論理Toolに対する永続Unlockとして定義した。
- Netherite Upgrade履歴をMariaDBへ保持する候補Fieldを追加した。
- Config変更でIron以下へ降格してもNetherite Upgrade履歴を失わないことを確定した。
- 累計Progressが再びDiamond条件へ到達した場合、WMを再請求せず自動的にNetheriteへ復帰することを確定した。

---

## 32. v0.0.3結論

Config再計算では修理を行わず、ACTIVE Toolの残存耐久率を新Materialへ換算する。換算後の残存耐久値は最低`1`とし、再計算だけでToolをBROKEN化させない。

Netherite Upgradeは一度成功した事実を永続的に保持する。Threshold変更によってToolがIron以下へ降格している間は基礎Materialを適用するが、再びDiamond条件へ到達した時点でNetheriteへ自動復帰する。
