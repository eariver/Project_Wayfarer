# Project Wayfarer Growth Tool Concept v0.0.1
...
MainへのPlayer Join時、`Wayfarer_Main`は非同期で次を照会する。
...
1. Race-safeにTool Recordを作成
2. Main Threadへ戻る
3. Wooden Growth Pickaxeを発行
4. Auditを記録
...
### 13.1 Gameplay Evolution

Block破壊によってEvolution Countが1以上増加した場合:
...
Threshold／上限変更による再計算は、無料修理として扱わない。

暫定方針:

- ACTIVE Toolは残存耐久率を維持して新Materialへ変換
- 新Materialの最大耐久に対してDamageを再計算
- BROKEN ToolはBROKENのまま
- 降格・昇格の双方で累計Progressは維持
- Config ReconciliationをAudit

この方針はPlaytest前に再確認可能とする。
...
- Config変更で完全再計算
- 必要量増加時に降格
- Gameplay Evolution時に最大回復
- Config Reconcile時に無料修理しない
...
Growth Pickaxeは累計Progressを正本とし、現行ConfigからMaterial、EnchantおよびEvolution Countを完全再計算する。
