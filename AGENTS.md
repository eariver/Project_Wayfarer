...
- Phase 1B must build the Builder command-focused allowlist after both Frontier Themes, the selected Gate／Portal permission model, MVI, ResourcePackManager, EliteMobs, custom-Plugin boundaries, and Builder Hub／Gate work are known. Do not grant MVI Group management, Resource Pack management, EliteMobs internals, custom-Plugin administration, WorldGuard Region administration, Velocity, LuckPerms, economy, player punishment, server stop, destructive World lifecycle, Plugin／Config reload, debug／internal administration, or unrestricted wildcards.
