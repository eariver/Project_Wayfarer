...
- Phase 3 Main generation completed on 2026-07-21 with seed `164225356311935743`. Persistent storage is `servers/main/main/dimensions/minecraft/{overworld,the_nether,the_end}`. This remains the current Runtime source of truth until the Ver.0.0.6 replacement generation, acceptance, tuning, backup, and baseline update complete.
...
- The Main persistent family is scheduled for a separately approved destructive regeneration only after all approved Content loads. Keep the current baseline authoritative until the replacement baseline is fully generated, accepted, tuned, backed up, and documented. Never include `resource`, `resource_nether`, or `resource_end` in this regeneration.
- The replacement Main generation is a one-time Ver.0.0.6 operation allowed only by a separately and explicitly assigned destructive task. It requires exact resolved paths, verified backup and rollback evidence, normal shutdown, explicit user approval, and confirmed loading of all approved Content; routine Resource reset scripts, daily operations, generic reset commands, and ambiguous reset requests never authorize it.
