...
- The existing LuckPerms `wayfarer_builder` group is the WorldGuard member allowed to build in protected entry worlds. Preserve that group and its Region membership during the Ver.0.0.4 permission-model implementation; do not delete, recreate, or use it as a Primary Group.
- In the current Runtime, WorldEdit and WorldGuard administration permissions for `wayfarer_builder` are restricted to `server=lobby` and `server=frontier`. The later Phase 1 task must audit those nodes, remove WorldGuard administration permissions, preserve Region membership, and rebuild the group to the Ver.0.0.4 Builder Allowlist.
...
- The Ver.0.0.4 permission model specifies five persistent group definitions: `default`, `wayfarer_builder_eligible`, `wayfarer_admin_eligible`, `wayfarer_builder`, and `wayfarer_admin`. Only a Player's Parent membership in `wayfarer_builder` or `wayfarer_admin` is temporary. This specification is not evidence that the missing groups or final permissions exist in the runtime database.
- `default` and `wayfarer_builder` already exist and must be audited and reused. After conflict checks, create `wayfarer_builder_eligible`, `wayfarer_admin_eligible`, and `wayfarer_admin` only if absent; if any already exists, audit it without deleting or replacing it.
...
- The planned Builder role is allowlist-based and the planned Admin role is full-access. Builder standard membership duration is 2 hours and Admin standard membership duration is 30 minutes, but neither duration is technically fixed in Ver.0.0.4. Permanent Player membership in either role and simultaneous temporary memberships are not the intended operating model.
- Builder WorldEdit, gamemode, and teleport access applies to Lobby, Main, and Frontier. Multiverse-Core access applies to all three Paper backends, while Multiverse-NetherPortals access applies only to Main. Use explicit command-focused allowlists verified during Phase 1; do not grant WorldGuard Region administration, Velocity administration, LuckPerms/economy/player-punishment/server-stop access, Multiverse world deletion/clone/regeneration/purge/import/unload, Plugin or Config reload, debug/internal administration, or unrestricted wildcards. Advanced Portals administration requires a later separate decision.
