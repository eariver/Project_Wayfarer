...
- The approved next Main BetterStructures scope is `103 Default Structures` version 5, `Exploration Pack` version 6, `Caves and Lost Civilizations Free` version 2, `Adventure Pack` internal version 1, `Echoes of the Past` version 3, BetterStructures Prop Pack, FreeMinecraftModels, and ResourcePackManager. These are planned targets, not verified Runtime artifacts; lock actual version, license, source, hash, and compatibility during implementation.
- Build a Runtime working copy, select Structures, normalize Prop／Model IDs, import every approved Plugin／Content artifact, and pass World-Allowlist, Content-load, and Resource Pack preflight before generating any replacement Main World.
...
- ResourcePackManager is the planned Main／Frontier distribution foundation. Main and Frontier use separate Packs. Lock Version, generation, Hosting URL, hash format, required policy, refusal／failure behavior, backend-switch reload, asset and model conflicts, size, cache, and rollback before implementation; generated Packs and Models remain ignored.
