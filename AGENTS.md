...
- The Ver.0.0.6 replacement generation completed on 2026-07-25 with seed `164225356311935743` and became the Final Main Baseline after Order 6 accepted the current Config without tuning and verified a stopped-state backup. Persistent storage remains `servers/main/main/dimensions/minecraft/{overworld,the_nether,the_end}`. This is not the V0.1.0 Release Baseline; Frontier, integrated operations, cold-backup／restore, the pre-release Player State reset, and other Release Blockers remain incomplete.
...
- The Final Main Baseline stopped-state backup is the verified ignored local backup `backups/main-v006-final-baseline-20260725-220745/`, with manifest SHA-256 `A85A7CCAA2FE2DECCC69CE3E9F862F1281408B4D02E20EFC1E3E31B74D0814A1` and SHA-list digest `81593864B49E41FB03F02514C1935DFAF380A1ABFE6FB148897932E328A50C39`. It contains 207 Main-container files totaling 148,994,081 bytes plus Config and evidence snapshots. The replacement rollback source remains `backups/main-v006-replacement-generation-20260725-154934/`; the 2026-07-21 legacy rollback baseline remains `backups/main-final-generation-20260721-001501/`. These backups are not stored in Git or guaranteed in another clone.
...
- Routine Resource reset scripts and normal or ambiguous reset instructions may target only `resource`, `resource_nether`, and `resource_end`. They must reject `main`, `main_nether`, and `main_the_end`.
- Runtime namespaced world keys are authoritative; documentation aliases are not filesystem paths.
- BetterStructures 2.6.3 is currently installed only on Main with WorldEdit 7.4.4 as its hard dependency. The approved V0.1.0 Ruined Frontier design also requires a separately verified Frontier installation; nothing in Ver.0.0.6 installs it.
- Main BetterStructures has the load-verified `main-betterstructures-v006` working set: `103 Default Structures` version 5, `Exploration Pack` version 6, `Caves and Lost Civilizations Free` version 2, `Adventure Pack` internal version 1, and `Echoes of the Past` version 3. Exactly 278 of 430 Structure configurations are enabled; 152 are disabled. Original, normalized, and generated Content remains ignored and is not redistributed.
...
- BetterStructures places structures only in newly generated chunks. Order 6 retained the default `spawnProtectionRadius: 100` and all current Weight／Distance／Content settings without modification.
- Natural generation is accepted from four distinct Main Packs: Default, Caves and Lost Civilizations Free, Echoes of the Past, and Exploration Pack. The fourth-Pack test added six Overworld Region files, establishing the Final Main Baseline counts at `17 / 11 / 18`; Nether, End, and every Resource Region were unchanged. Do not rerun that bounded exploration task.
- FreeMinecraftModels 2.10.2 and ResourcePackManager 2.3.0 are installed only on Main. FMM loads only the 55-Model BetterStructures Prop Pack; its furniture shop and general-player menu are disabled. ResourcePackManager merges the Main Java pack from ResourcePackManager, FMM, and BetterStructures. Neither Plugin nor the Prop Pack is installed on Velocity, Lobby, or Frontier.
- The Main working-copy pipeline, exact Structure selection, Prop／Model normalization, five-Pack import, World allowlist, Content load, and Resource Pack preflight completed on 2026-07-25. Reuse only the tracked manifests and scripts with the exact locked artifact hashes; do not edit original Content or commit normalized output.
- The tracked normalization removes exactly one saved egg Item and 144 stale `minecraft:bed` Block Entity records from the generated working copy under explicit user approval. Do not broaden either removal rule; source archives and ordinary saved Entities remain unchanged.
- The one-time Ver.0.0.6 replacement generation, Order 5 acceptance, and Order 6 Final Main Baseline task are complete. Never rerun them or include `resource`, `resource_nether`, or `resource_end` in any Main lifecycle operation. Future Weight／Content tuning, world lifecycle work, or backup replacement requires a new task based on the Final Main Baseline.
- The replacement operation was authorized only by the assigned destructive task and exact approval token, with resolved paths, verified backup／rollback evidence, normal shutdown, and confirmed Content load. Routine Resource reset scripts, daily operations, generic reset commands, and ambiguous reset requests do not authorize another run.
...
- V0.1.0 requires both Ruined Frontier alpha and Worlds Beyond MVP. Both Themes, their shared foundation, integration, and acceptance are Release Blockers; a World Generator or one Theme alone is insufficient.
- Worlds Beyond is exactly one persistent Iris Overworld, `frontier_iris`. Do not create or register `frontier_iris_nether` or `frontier_iris_the_end`, and do not create Worlds Beyond Multiverse-NetherPortals links or fallback routes. Nether／End portal activation or travel must fail closed. Ruined Frontier remains a separate Overworld／Nether／End family.
- Frontier Order 8 is complete under approved Proposal `FRONTIER-LOCK-20260726-003`. `docs/15-frontier-runtime-lock.md` and `config/frontier-lock/*-lock.yml` are authoritative for selected Artifacts, World IDs, MVI groups, Gate method, Pack inputs, persistence, permissions, and Runtime boundaries. Approval is a static Lock, not installation or Runtime acceptance; any locked Version／hash／World ID／Gate／Pack input／scope change requires a new Lock Revision.
- Ruined Frontier's initial BetterStructures Content scope is `Exploration Pack`, `Caves and Lost Civilizations Free`, `Echoes of the Past`, `Adventure Pack`, BetterStructures Prop Pack, Free Elite Shrines, and Dungeoneering Modules Free. The full `103 Default Structures` pack is disabled there by default because Main already provides it and it would dilute the high-difficulty Content pool. Any later adoption of individual Structures requires a separate formal design and assigned task; do not call the Ruined scope a five-Pack.
- Multiverse-Inventories is the planned Runtime source of truth for Frontier normal Player State in the `neutral`, `worlds_beyond`, and `guild` groups. Wayfarer_Frontier must not store normal Inventory in MariaDB or reimplement MVI profile switching.
- Main and Frontier must not share or transfer any Vanilla or custom Item, Inventory, Armor, Offhand, Ender Chest, Vanilla XP, Health, Food, Theme Item, EliteMobs loot, Quest Item, or material. Network-shared outcomes are limited to Waymark, mcMMO, and separately approved item-independent achievements or rewards.
- ResourcePackManager 2.3.0 is the load-verified Main distribution foundation. Main preflight uses an optional prompt and temporary Nightbreak remote hosting without public Backend port, firewall, router, credential, or account changes; generated Packs, Models, and hosting URLs remain ignored. This is not the formal V0.1.0 hosting lock. Frontier still requires a separate Pack and integration task, followed by Main／Frontier backend-switch delivery acceptance.
...
- Wayfarer_Core, Wayfarer_Main, and Wayfarer_Frontier are required V0.1.0 external custom Plugins and Release Blockers. Wayfarer_Main is Main-only and owns the Growth Pickaxe scope. `Wayfarer_Frontier_EliteMobsMVI` is a conditional independent Frontier Runtime Artifact created only when the formal decision is `ADAPTER_REQUIRED`; it is not a permanent Wayfarer_Frontier internal module.
- All custom Plugin Source belongs in one external Gradle Multi-module Repository. The exact Repository name, Group ID, Package root, module directories, Versioning, CI, and release process require later approval. Do not create that Repository or any module from this documentation task.
- This repository may contain only integration contracts, installation instructions, Version constraints, Config, Permission／Database／API contracts, acceptance tests, release hashes, and operational procedures for external custom Plugins.
...
- The current formal document revision is Ver.0.0.6. It defines the completion conditions for the future `V0.1.0 Alpha`; it is not a server release, tag, or GitHub Release.
...
- Phase 1B must build the Builder command-focused allowlist only after Ruined Frontier, Worlds Beyond, Multiverse-Inventories, ResourcePackManager, EliteMobs, Wayfarer_Core, Wayfarer_Main, Wayfarer_Frontier, any conditional Adapter, the adopted Gate／Portal permission model, and the exact Builder-owned Hub／Gate／Theme connection work are known. Do not grant MVI Group management, Resource Pack management, EliteMobs internals, custom-Plugin administration, WorldGuard Region administration, Velocity, LuckPerms, economy, player punishment, server stop, destructive World lifecycle, Plugin／Config reload, debug／internal administration, or unrestricted wildcards.
...
- Lobby, Main, Frontier Gate, Beyond Gate, Guild Gate, Resource return-gate structures, and the Resource End outer-island structure remain unbuilt and unconnected in Ver.0.0.6. The user determines their appearance, coordinates, orientation, and safe arrivals before Codex configures routing.
- CoreProtect is not installed on Main or Lobby. CoreProtect CE 24.0 rejected Minecraft 26.2 at Runtime, so Roadmap Order 7 is deferred, non-blocking, and waiting for an upstream Minecraft 26.2-compatible Stable release. Main／Lobby CoreProtect is temporarily excluded from the V0.1.0 Release Blockers while Project Owner-only operation continues; Order 9 Plugin Repository foundation is the next Roadmap work after the completed Order 8 Lock. Hub／Gate construction may proceed without CoreProtect only with the Final Main Baseline backup retained, focused before／after backup or ignored Schematic evidence as appropriate, small controlled edit units, and post-construction WorldGuard protection. WorldGuard is preventive protection and does not replace history lookup or point-in-time rollback; CoreProtect also never replaces cold backup. Re-evaluate before other Players, multiple Builders, public operation, large multi-person WorldEdit work, or materially expanded persistent construction. Frontier Block History／Rollback remains product-unselected under the Order 8 Lock and requires a separate selection task.
- A separately approved destructive V0.1.0 pre-release reset is required after gameplay/portal/hub/permission/backup testing and immediately before the final baseline backup. Its minimum scope is all Waymark balances plus Main inventory, armor/offhand, Vanilla XP, and advancements; exact additional player-state scope remains undecided. Whether Growth Tool logical data is reset or preserved is explicitly undecided and requires separate Owner approval. Do not perform it early or edit Redis keys directly.
- The current Frontier Concepts are Frontier Server V0.0.5, Worlds Beyond V0.0.6, and Ruined Frontier V0.0.5. They are detailed, non-authoritative design inputs, not implementation tasks. Only the scope explicitly promoted into Ver.0.0.6 source-of-truth documents may be designed into separately assigned implementation tasks.
- Frontier Block History／Rollback remains unselected. CoreProtect CE 24.0 is an available static candidate for later validation on Paper 1.21.11, not an adopted Frontier product or installed Runtime component.
- Growth Pickaxe is a Main-only V0.1.0 Release Blocker owned by Wayfarer_Main. Its logical identity, owner binding, Resource-family-only progress, evolution, broken state, Waymark repair, pending delivery, audit, and reconcile data use MariaDB as authoritative storage; Redis and physical item state are not the sole source of truth. It must never be transferred to Frontier.
- `codex/Project_Wayfarer_V0.2x_Custom_Plugin_Concept.md` remains a non-authoritative historical concept for proposals not promoted into Ver.0.0.6. It is not itself a Runtime source of truth, implementation task, repository-creation approval, database-migration approval, or artifact-creation approval.
...
- During manual Main administration or acceptance work, do not leave the user waiting in Survival because hostile mobs may damage the Player or terrain. Keep the Player in Creative or Spectator while waiting, and return to Survival only immediately before an actual disconnect or completed handoff.
